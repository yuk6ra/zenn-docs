---
title: "【Solidity】Hardhatを利用して爆速でスマートコントラクト開発の環境を整える"
emoji: "🤖"
...
`hardhat.config.js`を開き、Goerliを想定する設定を記入していきます。`solidity`のバージョンはコントラクトで指定したバージョンを記入します。

随時、ライブラリを導入した際に設定を記入していくことになります。
