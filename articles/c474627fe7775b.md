...
本記事は、以下の公式ドキュメントやブログを参考に、特定のバージョンで動作確認を行っています。リポジトリの更新が速いため、同様の環境を再現する際はご注意ください。
...
-   **GitHub**: [NVIDIA/Isaac-GR00T](https://github.com/NVIDIA/Isaac-GR00T) / [huggingface/lerobot](https://github.com/huggingface/lerobot)
    - **Isaac-GR00T**: [`d598400`](https://github.com/NVIDIA/Isaac-GR00T/tree/d5984002e24d418872adc5822a5bbb1d6a9b4ddc)
    -   **LeRobot**: [`519b761`](https://github.com/huggingface/lerobot/tree/519b76110efeea55a4f919895d0029dc0df41e8b)
...
推論サーバーでクラウドを利用している場合は、`--policy_host`と`--policy_port`を設定してください。
...
**解決策:** 仮想環境が`gr00t`になっているかも再確認してください。

## 評価結果

実行が完了すると、モデルの予測（prediction）と実際の動作（ground truth）を比較したグラフが出力されます。このグラフにより、モデルがどの程度正確にタスクを学習できたかを視覚的に確認できます。
