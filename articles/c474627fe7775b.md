...
huggingface-cli download \
       --repo-type dataset yuk6ra/so101-tapes-cleanup \
       --local-dir ./demo_data/so101-tapes-cleanup
```


```sh
cp getting_started/examples/so100_dualcam__modality.json ./demo_data/so101-tapes-cleanup/meta/modality.json
```

今回は
```sh
vim ./demo_data/so101-tapes-cleanup/meta/modality.json
```
...
```
python scripts/load_dataset.py --dataset-path ./demo_data/so101-tapes-cleanup --plot-state-action --video-backend torchvision_av
```
...
========================================= Humanoid Dataset =========================================
====================================================================================================
{'action.gripper': 'np scalar: 1.1904761791229248 [1, 1] float64',
 'action.single_arm': 'np: [1, 5] float64',
 'annotation.human.task_description': ['Grab tapes and place into box'],
...
action.gripper: (1, 1)
annotation.human.task_description: ['Grab tapes and place into box']
Image 0, prompt: ['Grab tapes and place into box']
Image 10, prompt: ['Grab tapes and place into box']
Image 20, prompt: ['Grab tapes and place into box']
Image 30, prompt: ['Grab tapes and place into box']
Image 40, prompt: ['Grab tapes and place into box']
Image 50, prompt: ['Grab tapes and place into box']
Image 60, prompt: ['Grab tapes and place into box']
Image 70, prompt: ['Grab tapes and place into box']
Image 80, prompt: ['Grab tapes and place into box']
Image 90, prompt: ['Grab tapes and place into box']
Image 100, prompt: ['Grab tapes and place into box']
Image 110, prompt: ['Grab tapes and place into box']
Image 120, prompt: ['Grab tapes and place into box']
Image 130, prompt: ['Grab tapes and place into box']
Image 140, prompt: ['Grab tapes and place into box']
Image 150, prompt: ['Grab tapes and place into box']
Image 160, prompt: ['Grab tapes and place into box']
Image 170, prompt: ['Grab tapes and place into box']
Image 180, prompt: ['Grab tapes and place into box']
Image 190, prompt: ['Grab tapes and place into box']
...
python scripts/gr00t_finetune.py \
      --dataset-path ./demo_data/so101-tapes-cleanup/ \
      --num-gpus 1 \
...
```sh
  ...
  File "/venv/gr00t/lib/pythH100で30分ほど放置。

MAX5000ステップで100GBほどを消費。
on3.10/site-packages/accelerate/data_loader.py", line 552, in __iter__
...
huggingface-cli upload yuk6ra/so101-table-cleanup . --commit-message="5000 step"
```
...
huggingface-cli download yuk6ra/so101-tapes-cleanup --repo-type model --local-dir ./model/so101-tapes-cleanup
```
...
python scripts/eval_policy.py --plot \
       ...
       --model_path ./model/so101-tapes-cleanup/ \
       --data_config so100_dualcam \
       --dataset_path ./demo_data/so101-tapes-cleanup/ \
...
立ち上げる
```sh
python scripts/inference_service.py --model_path ./model/so101-tapes-cleanup --embodiment_tag new_embodiment --data_config so100_dualcam --server --port 5555
```
...
python getting_started/examples/eval_lerobot.py \
         ...
         --robot.id=white \
         --robot.cameras="{
             tip: {type: opencv, index_or_path: 0, width: 640, height: 480, fps: 30},
             front: {type: opencv, index_or_path: 2, width: 640, height: 480, fps: 30}
         }" \
         --lang_instruction="Grab tapes and place into box."
```
